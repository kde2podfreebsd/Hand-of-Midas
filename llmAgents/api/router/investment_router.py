...
from llmAgents.api.schema.investment_survey import InvestmentQuestion, InvestmentSurveyResponse
from llmAgents.api.service.investment_service import InvestmentService
...
def get_investment_service() -> InvestmentService:
    ...
    db_name = "chat_db"
    return InvestmentService(mongodb_uri, db_name)

@router.get("/survey/questions", response_model=List[str])
async def get_survey_questions(service: InvestmentService = Depends(get_investment_service)):
    return [
...
async def get_survey_answers(user_id: str, service: InvestmentService = Depends(get_investment_service)):
